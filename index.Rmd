...
FoodInsecurity %>% 
  mutate(
    `County Name` = recode(`County Name`, "New York" = "New York County",                                "Richmond" = "Richmond County")) %>% 
  rename("countyfp" = CountyFIPS)
```

```{r include = FALSE}
BoroughBoundaries = 
  readOGR(dsn = './data/nyu_2451_34510/nyu_2451_34510.shp', stringsAsFactors = FALSE)
```

```{r}
JoinedMapData =  
  merge(BoroughBoundaries, FoodInsecurityClean, by.x = "countyfp", by.y = "countyfp")
```


```{r}
leaflet::leaflet(data = JoinedMapData) %>% 
    addTiles() %>%
    addPolygons()
```
